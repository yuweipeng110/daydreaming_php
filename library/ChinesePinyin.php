...
class ChinesePinyin {
	...
	public function __construct() {
		if (empty ( $this->ChineseCharacters )) {
			$this->ChineseCharacters = file_get_contents ( DOCSDIR . '/ChineseCharacters.dat' );
		}
	}
	
	/*
	 * 转成带有声调的汉语拼音 param $input_char String 需要转换的汉字 param $delimiter String 转换之后拼音之间分隔符 param $outside_ignore Boolean 是否忽略非汉字内容
	 */
	...
	/*
	 * 转成带无声调的汉语拼音 param $input_char String 需要转换的汉字 param $delimiter String 转换之后拼音之间分隔符 param $outside_ignore Boolean 是否忽略非汉字内容
	 */
	public function TransformWithoutTone($input_char, $delimiter = '', $outside_ignore = true) {
		...
		$char_without_tone = str_replace ( array (
				'ā',
				'á',
				'ǎ',
				'à',
	...
	/*
	 * 转成汉语拼音首字母 param $input_char String 需要转换的汉字 param $delimiter String 转换之后拼音之间分隔符
	 */
