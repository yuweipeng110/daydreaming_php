...
(function(win, doc, undefined) {
	var settings, body, fakeBody, windowLoaded, head, docElem = doc.documentElement, testPass = false, mediaCookieA, mediaCookieB, toggledMedia = [];
	...
	win.enhance = function(options) {
		...
		for ( var name in enhance.defaultSettings) {
			...
			settings[name] = option !== undefined ? option
					: enhance.defaultSettings[name];
		}
		// mixin additional tests
		for ( var test in options.addTests) {
			settings.tests[test] = options.addTests[test];
		}
		// add testName class immediately for FOUC prevention, remove later on
		// fail
	...
	enhance.defaultTests = {
		...
		floatClear : function() {
			var pass = false, newDiv = doc.createElement('div'), style = 'style="width: 5px; height: 5px; float: left;"';
			newDiv.innerHTML = '<div ' + style + '></div><div ' + style
					+ '></div>';
			body.appendChild(newDiv);
			var childNodes = newDiv.childNodes, topA = childNodes[0].offsetTop, divB = childNodes[1], topB = divB.offsetTop;
			if (topA === topB) {
		...
		ajax : function() {
			...
			var xmlhttp = false, index = -1, factory, XMLHttpFactories = [
					function() {
	...
	function appendToggleLinks(result) {
		...
		if (result) {
			...
			a.innerHTML = result === 'pass' ? settings.forceFailText
					: settings.forcePassText;
			a.onclick = result === 'pass' ? enhance.forceFail
					: enhance.forcePass;
	...
	function appendStyles() {
		...
		while ((item = settings.loadStyles[++index])) {
			...
			if (typeof item === 'string') {
				...
				var applies = true;
				if (item['media'] && item['media'] !== 'print'
						&& item['media'] !== 'projection'
						&& item['media'] !== 'speech'
						&& item['media'] !== 'aural'
						&& item['media'] !== 'braille') {
	...
	var isIE = (function() {
		...
		return function(condition) {
			...
			var cc = 'IE';
			if (condition) {
				if (condition !== 'all') { // deprecated support for 'all'
					// keyword
					if (!isNaN(parseFloat(condition))) {
						cc += ' ' + condition; // deprecated support for
						// straight version #
					} else {
						cc = condition; // recommended (conditional comment
						// syntax)
					}
				}
			}
	...
	var mediaquery = (function() {
		...
		return function(q) {
			...
			if (cache[q] === undefined) {
				...
				/*
				 * set inner css text. credit:
				 * http://www.phpied.com/dynamic-script-and-style-elements-in-ie/
				 */
				var cssrule = '@media '
						+ q
						+ ' { #ejs-qtest { position: absolute; width: 10px; } }';
	...
	function appendScriptsSync() {
		...
		function next() {
			...
			var item = queue.shift(), script = createScriptTag(item), done = false;
			if (script) {
